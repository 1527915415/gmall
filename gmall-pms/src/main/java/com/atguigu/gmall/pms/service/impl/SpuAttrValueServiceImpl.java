...
public class SpuAttrValueServiceImpl extends ServiceImpl<SpuAttrValueMapper, SpuAttrValueEntity> implements SpuAttrValueService {
    ...
    @Override
    public List<SpuAttrValueEntity> querySearchSpuAttrValueBySpuAndCid(Long spuId, Long cid) {
        ...
        List<Long> attrs = attrEntityList.stream().map(AttrEntity::getId).collect(Collectors.toList());
        return this.list(new QueryWrapper<SpuAttrValueEntity>().eq("spu_id",spuId).in("attr_id",attrs));
    }
